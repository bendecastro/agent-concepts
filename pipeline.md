...
```
  ...
  ready-for-agent issue queue  ◄──────────────────────────────────────────┘
   ...
   │       └► fresh subagent: tdd OR diagnosing-bugs ─► validate ─► commit ─► push master ─► close
...
**Execution — `/bc-drain-issues`** (AFK, run after planning/triage). A preflight-gated driver loop that atomically claims each unblocked issue, dispatches a **fresh subagent** to build the slice test-first (or run `diagnosing-bugs` for bugs) and land it **trunk-based** (commit → push `master` → close with a validation comment), **parking** cleanly on failure and tripping a circuit-breaker on a run of parks. The only human gate is the preflight; everything human happens before the loop starts.
...
| Model-invoked disciplines | `grilling`, `domain-modeling`, `prd-drafting`, `issue-slicing`, `tdd`, `diagnosing-bugs`, `codebase-design`, `bc-autoresearch-loop` |
