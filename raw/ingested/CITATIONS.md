...
| `unclebob-quality-tools` | [unclebob/negative-test-experiment](https://github.com/unclebob/negative-test-experiment) + [Acceptance-Pipeline-Specification](https://github.com/unclebob/Acceptance-Pipeline-Specification) + the `crap4*`/`mutate4*`/`dry4*` tools + [swarm-forge](https://github.com/unclebob/swarm-forge) | **No open licence** (`Copyright (c) Robert C. Martin. All rights reserved.`); read via API/raw 2026-08-20, nothing cloned or vendored. Ingested → acceptance-mutation (2026-08-20), plus the “complexity scores are not a quality bar” rule in code-review, which inverts the CRAP threshold against the experiment's own grid. SwarmForge role packs, the handoff daemon, and the Clean Code teaching repositories deliberately not filed. |
