...
Before changing a concept's behavioral contract, read its `RELATIONSHIPS.md` and account for every incoming edge: update the affected consumer in the same change, or record why it is unaffected. Why: that generated view is the only place a callee can see its callers, so an unread incoming edge is a caller you broke without noticing. When a clause adds, removes, or changes a `loads`, `adapts`, `depends_on_contract`, or `hands_off` relationship, edit `concepts/relationships.json` in the same change and regenerate with `python3 scripts/lint.py --write-relationships`. Never hand-edit a generated view — the next regeneration reverts it, so the fix disappears exactly when someone starts trusting it.
