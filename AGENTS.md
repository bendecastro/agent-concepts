...
- **Bulk local concepts:** run `scripts/deploy-local-skills.py` from this workspace. It exposes every `concepts/*/body/SKILL.md` through `~/.agents/skills/<name>`, `~/.pi/agent/skills/<name>`, and `~/.claude/skills/<name>` using relative symlinks.
...
- **Other agents:** see `bootstrap.md` and `harnesses.md` — Codex/OpenCode/Gemini still use AGENTS.md deltas or manual bootstrap until a native skills path is verified.
