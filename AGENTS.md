...
- Commit changes to this directory in this repo (not CONFIG) with a message saying which operation ran. Publishing follows `~/.config/agent-concepts/publish.yaml`, which is the only home for that authorization. Do not restate its rules here: this line once named `agent-concepts-push-on-explicit-instruction`, a rule the policy no longer contains, and so blocked pushes the policy required. Get the objective path/remote/branch answer from `python3 scripts/publish-check.py --repo "$PWD" --remote "$(git remote get-url origin)" --branch "$(git branch --show-current)"`, then verify the matched rule's `when:` conditions yourself before pushing.
