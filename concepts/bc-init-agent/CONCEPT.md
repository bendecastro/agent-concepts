...
User-invoked adaptive initializer that first inspects a folder/repo, grills only on the decisions that matter for that environment, and then stands up a project-local agent workspace — a repo-root `AGENTS.md` pointing agents at the vault, plus a `.bc-agent/` Obsidian-vault wiki (maintainer schema, templates, conventions, ADR-0001, the `planning-workflow.md` adapter, and a repo-local loop skill map), plus `.bc-agent/out-of-scope/` for triage memory — so the repo is ready for the intake→grill→issues→drain loop. The setup step before `triage` / `bc-plan-to-issues`. The `bc-` prefix is the user's personal namespace.
...
- **Deterministic scaffolding via a script.** File-tree generation from a fixed template is exactly the kind of thing an LLM does unreliably, so `body/scaffold.py` writes the 25-file tree with `__SLUG__`/`__DATE__` substitution. The skill body handles what needs judgment: environment recon, targeted grilling, plan proposal, the clobber guard's follow-up, conservative seed edits, the publish.yaml offer, and the commit. (AGENTS.md "scripts for anything an LLM does unreliably".)
