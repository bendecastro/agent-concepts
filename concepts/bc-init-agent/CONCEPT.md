...
User-invoked scaffolder that stands up a project-local agent workspace — a repo-root `AGENTS.md` pointing agents at the vault, plus a `.bc-agent/` Obsidian-vault wiki (maintainer schema, templates, conventions, ADR-0001, and the `planning-workflow.md` adapter), plus `.out-of-scope/` for triage memory — so a fresh repo is immediately ready for the intake→grill→issues→drain loop. The setup step before `triage` / `bc-plan-to-issues`. The `bc-` prefix is the user's personal namespace.
...
- **Triage-ready memory.** The scaffold now creates `.out-of-scope/.gitkeep` so the `triage` skill has a predictable place for durable rejected enhancement concepts without mixing them into `.bc-agent/` methodology docs.
