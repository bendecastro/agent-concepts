...
- **Clobber guard.** The script refuses (exit 2) if `.bc-agent/` exists, and only writes the root `AGENTS.md` if absent — a live vault or a hand-written root AGENTS.md is never overwritten. Standing up a workspace must be safe to re-attempt.
