...
User-invoked scaffolder that stands up a project-local agent workspace — a repo-root `AGENTS.md` pointing agents at the vault, plus a `.bc-agent/` Obsidian-vault wiki (maintainer schema, templates, conventions, ADR-0001, and the `planning-workflow.md` adapter), plus `.bc-agent/out-of-scope/` for triage memory — so a fresh repo is immediately ready for the intake→grill→issues→drain loop. The setup step before `triage` / `bc-plan-to-issues`. The `bc-` prefix is the user's personal namespace.
