...
2. **Guard against clobber.** The script refuses (exit 2) if `.agent/<slug>/` already exists — never overwrite a live vault. If it refuses, stop and offer to extend a specific missing page by hand, or use a different slug.

3. **Scaffold.** Run the bundled script (it lives next to this file):
   ```
   python3 <skill-dir>/scaffold.py --root "<repo-root>" --slug "<slug>"
   ```
   It writes the root `AGENTS.md` (only if absent — if one exists it tells you to merge the vault pointer by hand) and the full `.agent/<slug>/` tree with generalized schema + TODO stubs (`validation.md`, `file-layout.md`, `references/*`, the glossary) for the project's agents to fill as they learn the repo.
