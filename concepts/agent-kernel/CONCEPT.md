...
- **Offer to push after committing** — the never-publish-by-default rule prevents *unwanted* pushes but used to let a finished commit sit silently, leaving the user unsure whether to push. The kernel now requires proactively asking whether to push after a commit, while still gating the push itself on explicit yes / policy. The user commits loosely and often but wants to remain the per-commit decision-maker on what reaches the remote. (Added 2026-06-26 from a user request, generalized cross-vendor.)
