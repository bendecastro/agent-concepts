...
- Size for the executor's whole issue lifecycle, not implementation alone: audit + build + independent review + likely rework must plausibly finish below `/bc-drain-issues`' current 200k aggregate child-token soft cap. This is an agent-readiness gate, not optional polish: when that fit is doubtful, split at independently verifiable behavior boundaries, even if asked to keep one issue or "use the 300k limit." If the user rejects the split, say the unsplit item is not drain-ready instead of handing it back as agent-ready. Do not invent per-slice token estimates; classify fit qualitatively from scope, compatibility surface, external semantics, migration risk, and likely rework. The 300k hard cap is a recovery boundary, never a sizing target.
- Any prefactoring is its own first slice.
</vertical-slice-rules>

### 4. Quiz the user
Present the breakdown as a numbered **draft**, never as finalized work. For each slice show: **Title**, **Blocked by** (which slices must finish first), **User stories covered**, and **Drain budget fit** (`plausibly below the 200k soft cap` plus a brief scope/risk reason; never an invented token estimate). Ask: Does the granularity feel right (too coarse / too fine)? Are the dependency relationships correct? Should any slices merge or split? Iterate until the user approves. **Do not finalize, publish, or treat “I trust you” / “skip the review” as approval** — the explicit quiz is the human gate before slices become agent-ready work.
