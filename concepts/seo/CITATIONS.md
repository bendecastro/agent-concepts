...
Maps the load-bearing claims in `body/` to the primary sources held in `../../ideas/seo-primary-sources/` (each file's header carries its origin URL). Maintained with the concept: when a body claim changes, update its row; when a row has no held source, that's a lint-worthy gap. Snapshot dates matter — these documents evolve; re-verify at runtime for time-sensitive use, per the body's own rules.
