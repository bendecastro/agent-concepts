...
a copy written by `notebooklm skill install` from the installed [teng-lin/notebooklm-py](https://github.com/teng-lin/notebooklm-py) package. There is no auto-updater: after `uv tool upgrade notebooklm-py`, rerun `notebooklm skill install` to refresh it. See `../CONCEPT.md` for the rationale, and `../../../docs/research/raw/ingested/notebooklm-skill-upstream/` for the cited snapshot.
