...
# Reporting/tune.
report_data={'states':{'LANDED':[29],'HUMAN_BLOCKED':[40],'REWORK_DEFERRED':[41,42],'SYSTEMIC_FAILURE':['run token circuit']},'parent_status':{'10':'closed after all complete','20':'open: child deferred'},'stop_reason':'two consecutive token deferrals','child_runs':[{'issue':29,'role':'audit','model':'GPT-5.6 Luna max','usd':0.04},{'issue':29,'role':'build','model':'GPT-5.6 Luna max','usd':0.27},{'issue':29,'role':'review-combined','model':'Grok 4.6 high','usd':0.17},{'issue':29,'role':'rework','model':'GPT-5.6 Luna max','usd':0.43},{'issue':40,'role':'build','model':'GPT-5.6 Luna max','usd':'unavailable'}],'list_api_estimate_usd':0.91,'round_counts':{'29':3},'repeated_finding_events':[{'issue':29,'attempts':2,'action':'defer'}],'full_suite_counts':{'base_'+BASE:1,'final_landing':1},'recovery':{'29':'validated exact','41':'portable brief only'},'packet_tune':{'trigger':'same missing baseline-delta evidence in two workers','patch':'+ Explicitly name every known failing ID and unchanged/regressed status.','additive_only':True,'canon_edited':False,'gates_weakened':False},'stale_resources':[],'main_checkout_clean':realgit('status','--porcelain',cwd=REPO).stdout==''}
dump('17/end-report.json',report_data)
assert_('child_runs' in report_data and 'list_api_estimate_usd' in report_data and 'tokens' not in report_data and 'caps_crossed' not in report_data,'end-report still carries token counts')
assert_('List-API estimate' in skill_text and 'per-issue and total child tokens' not in skill_text,'SKILL still reports token totals')
...
# --- Acceptance evidence must discriminate (not merely be present) ---
for required in ('Acceptance evidence must discriminate','would look different if the criterion were false','not a full-matrix audit'):
 assert_(required in contract,f'review contract missing discriminating-evidence rule: {required}')
assert_('observable evidence that would differ if the criterion were false' in worker_contract,'worker contract does not require discriminating evidence')

# Rows carry evidence that the deterministic gate accepts; only the Spec reviewer can tell
# whether that evidence would change had the criterion been false.
EVID={
 'acceptance-1':{'row':'tool.sh --new-flag exits 0 and prints the new banner','cited':'test_new.sh::new_flag PASS','would_differ_if_false':True,'implicated':True},
 'acceptance-2':{'row':'service unit is installed and enabled','cited':'systemctl list-unit-files | wc -l == 42','would_differ_if_false':False,'implicated':True},
 'acceptance-3':{'row':'launcher resolves through the installed symlink','cited':'round-1 evidence','would_differ_if_false':False,'implicated':False},
}
gate_view={r:{'has_evidence':bool(e['cited'])} for r,e in EVID.items()}
assert_(all(v['has_evidence'] for v in gate_view.values()),'fixture must pass the deterministic presence gate')
spec_findings=[{'row':r,'severity':'Important','why':'evidence identical whether or not the criterion holds'}
               for r,e in EVID.items() if e['implicated'] and not e['would_differ_if_false']]
assert_([f['row'] for f in spec_findings]==['acceptance-2'],'Spec did not flag non-discriminating evidence on an implicated row')
assert_(all(f['row']!='acceptance-3' for f in spec_findings),'untouched row was audited; rule must not become a full-matrix sweep')
remedy={'row':'acceptance-2','rejected':'attach a second command with the same property','accepted':'assert the unit is enabled, which fails when it is not'}
...
# --- Drain-side architecture observation handoff ---
# This case is intentionally contract-driven: the fixture models the producer and sink
# boundaries, while the assertions below make the non-authoritative handoff inspectable.
for required in (
 ...
 'never cite an unlanded worker diff',
 '`.bc-agent/research/architecture-observations.md`',
 'explicitly declared equivalent path in project instructions',
 'separate driver-owned context-only update after landing',
 'outside the review packet and implementation diff',
 'does not dispatch a child',
 'cannot affect review, rework, tier, approval, landing, labels, or issue state',
 'Report the inbox path and context commit',
 '`not persisted` with the reason',
):
...
def make_observation(issue):
 return {
  ...
  'interface_or_seam':'`billing` interface / `ledger` seam',
  'friction':'billing duplicates ledger normalization across modules',
  'deletion_test':'deleting the billing helper makes normalization reappear across callers',
  'evidence':'tests/billing_ledger.py::test_normalization_cross_module',
  'status':'open',
 }
...
configured_rel=Path('.bc-agent/research/architecture-observations.md')
configured_sink=ART/'sinks/configured'/configured_rel
configured_sink.parent.mkdir(parents=True,exist_ok=True)
configured_header='# Architecture observations'
older_issue={'number':99,'state':'LANDED','commit':'abc099','tree':'tree099','shape_only':False,'structural_signal':'duplicate'}
older_observation=make_observation(older_issue)

def format_observation_entry(issue_number,observation):
 lines=[f'## issue #{issue_number}']
 lines.extend(f'- {key}: {observation[key]}' for key in OBS_FIELDS)
 return '\n'.join(lines)+'\n'

configured_sink.write_text(f'{configured_header}\n\n{format_observation_entry(older_issue["number"],older_observation)}')
configured_before=configured_sink.read_text()
context_events=['implementation-commit','review-packet','landing']

def persist_observation(observation,sink):
 if observation is None:
  return {'status':'not applicable'}
 if not sink['declared'] or not sink['exists'] or not sink['writable']:
  return {'status':'not persisted','reason':sink['reason']}
 existing=sink['path'].read_text()
 header,separator,prior_entries=existing.partition('\n\n')
 if not separator:
  header,prior_entries=existing.rstrip('\n'),''
 new_entry=format_observation_entry(101,observation)
 contents=f'{header}\n\n{new_entry}'
 if prior_entries.strip():
  contents+=f'\n{prior_entries.lstrip()}'
 sink['path'].write_text(contents)
...
new_heading='## issue #101'
older_heading=f"## issue #{older_issue['number']}"
configured_ordering={
 'header':configured_header,
 'new_heading':new_heading,
 'older_heading':older_heading,
 'header_index':configured_text.index(configured_header),
 'new_observation_index':configured_text.index(new_heading),
 'older_observation_index':configured_text.index(older_heading),
}
configured_ordering['newest_first']=configured_ordering['new_observation_index'] < configured_ordering['older_observation_index']
...
assert_(configured_text.count('## issue #101')==1,'configured inbox contains more than one observation for the issue')
assert_(configured_text.count(older_heading)==1,'configured inbox lost or duplicated the older observation')
assert_(configured_text.startswith(f'{configured_header}\n\n'),'configured inbox header was not preserved')
assert_(configured_ordering['header_index']==0 and configured_ordering['newest_first'],'new observation was not prepended newest-first before the older entry')
...
dump('28/architecture-observation.json',{
 ...
 'configured_sink':{'report':configured,'seeded_content':configured_before,'content':configured_text,'ordering':configured_ordering,'separate_driver_context':True},
