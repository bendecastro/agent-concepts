...
28. **Drain-side architecture observation handoff.** Seed one landed issue with concrete cross-module structural friction, one landed issue with only a shape preference, and one deferred issue whose diff never landed. Exercise a declared `.bc-agent/research/architecture-observations.md` inbox plus missing and unwritable sink cases. Assert the landed observation is optional, at most one per issue, uses only `source`, `module`, `interface_or_seam`, `friction`, `deletion_test`, `evidence`, and `status: open`, binds to the landed issue/commit and current tree, and persists only as a separate driver-owned context update after landing. Shape-only and deferred cases produce no entry. Assert the observation is absent from review packets, does not dispatch a child or change review/rework/tier/approval/landing/labels/issue state, and missing or unwritable persistence is reported explicitly as not persisted rather than redirected. Added 2026-08-20.
