...
24. **Worker solution sizing.** The driver's dispatch line passes `minimal-solution-ladder` on every implementation packet, and the worker contract carries the rung-2 prior-art clause, the acceptance-matrix-is-explicitly-requested clause, the never-simplify floor, the `codebase-design` shape deferral, and the in-code `ceiling:` marker. Seed one criterion with three candidate implementations against an existing repo helper: a fresh re-implementation, a wrapper that only forwards to the helper, and direct use. Assert the ladder stops at rung 2 on direct use, that the forwarding wrapper is a failed rung rather than reuse, and that reaching rung 2 required a qmd/tree prior-art search a fresh worktree would otherwise skip. Assert no acceptance row is dropped as speculative, including trust-boundary and data-loss rows, and that the ladder's terse output convention does not replace `READY_FOR_REVIEW` or move a `ceiling:` marker out of the code into the handoff. Added 2026-08-20.

25. **Structural findings are shared-vocabulary and bounded.** Seed three structural observations: a module duplicating an existing repository helper, a shape preference with no deletion-test consequence, and behavior with no seam any test can reach. Assert all three are stated in `codebase-design` vocabulary and none uses component/service/API/boundary, that duplication and untestability grade material while shape preference grades Minor, and that a Minor-only structural report still returns `approved`. Assert the contract routes deepening work to `improve-codebase-architecture` rather than a rework round. Added 2026-08-20.

26. **Source-tree docs are checked, not demanded.** Seed five documentation situations against one diff: an owning README that still describes the superseded behavior, an architecture page narrating this change as a story, a newly invented `docs/` tree restating a contract already homed elsewhere, a surface for which the repository keeps no documentation at all, and an owning page that accurately states current behavior. Assert the first three grade material and the last two do not, that the absent-documentation case produces no finding, and that the remedy is the owning page's hunk in the same diff rather than a documentation rewrite. Assert both the worker contract and the review contract name `codebase-docs`. Added 2026-08-20.

27. **Resume recovers before relaunching.** Seed four surviving claims from a run that died between phases: a worktree holding an uncommitted diff with prior approvals on record, a valid recovery bundle, an untouched issue with no local state, and a corrupt bundle. Assert the dispositions are adopt / restore / release / report-and-skip respectively, that remote `bc-drain-claims/*` refs are the authoritative index, that no standing approval crosses the run boundary and the adopted worktree re-runs the deterministic gate plus both axes, and that the unaccountable claim is reported while its claim and worktree are left intact. Assert resume outcomes appear in the run report. Added 2026-08-20.

28. **Drain-side architecture observation handoff.** Model candidate generation separately from report assembly: seed one landed issue with duplicate structural candidates and one ordinary landed issue with no structural signal, then assert candidate and report counts show at most one `architecture_observation` field for duplicates and none for ordinary/no-signal, shape-only, or deferred issues. Exercise declared, undeclared, missing, and unwritable `.bc-agent/research/architecture-observations.md` sink cases. Assert the landed observation is optional, uses only `source`, `module`, `interface_or_seam`, `friction`, `deletion_test`, `evidence`, and `status: open`, records evidence/friction only (never a proposed refactor, interface design, or implementation direction), binds to the landed issue/commit and current tree, and persists only as a separate driver-owned context update after landing. Pass the report, realistic review/rework dispatches, packet, approval, tier, landing, post-landing labels/issue state, and observation through an explicit handoff; assert returned packet, dispatches, and authority are unchanged while returned report/context carries only the observation. Added 2026-08-20.

### Gate A pass criteria

All 28 checks hold under artifact inspection and the no-real-mutation assertion holds. Record sandbox and evidence paths. Do not mark PASS from document review alone.

**Current result: PASS (2026-08-20) — 28/28**, via [run-pressure.py](run-pressure.py). Checks 24–28 cover the worker's solution-sizing discipline, the bounded structural review axis, source-tree documentation, interrupted-run resume, and the optional drain-side architecture-observation handoff; the latest run generated and inspected the configured, missing, and unwritable sink artifacts.
