...
28. **Drain-side architecture observation handoff.** Seed one landed issue with concrete cross-module structural friction, one landed issue with only a shape preference, and one deferred issue whose diff never landed. Exercise declared, undeclared, missing, and unwritable `.bc-agent/research/architecture-observations.md` sink cases. Assert the landed observation is optional, at most one per issue, uses only `source`, `module`, `interface_or_seam`, `friction`, `deletion_test`, `evidence`, and `status: open`, records evidence/friction only (never a proposed refactor, interface design, or implementation direction), binds to the landed issue/commit and current tree, and persists only as a separate driver-owned context update after landing. Shape-only and deferred cases produce no entry. Assert the observation is absent from a realistic review packet, does not dispatch a child or change the seeded review dispatch, tier, approval, landing, labels, or issue state, and undeclared, missing, or unwritable persistence is reported explicitly as not persisted with no fallback write. Added 2026-08-20.
