...
- **Medium effort by default for per-issue subagents.** The driver explicitly avoids inheriting a high/xhigh parent reasoning setting. Medium is the cost-aware default for repeat AFK TDD slices: enough headroom for tests, rebases, and validation, without turning every issue into a high-effort run.
