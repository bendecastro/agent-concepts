...
| Grok | Deployed 2026-07-03 via `scripts/deploy-grok-skills.py` | **User symlinks:** `~/.grok/skills/{design,execute-plan,implement,pr-babysit,review,shared}` → `~/.grok/bundled/skills/…`; `code-review` + `strict-code-review` → `~/Sync/CONFIG/agents/concepts/strict-code-review/body`. **Also native:** `check-work`, `create-skill`, office/help skills under `~/.grok/skills/`. **Bundled path** still holds live xAI copies. | Native Agent Skills metadata (`when-to-use`, `disable-model-invocation`) | Grok-specific tools (`spawn_subagent`, `grok worktree rm`, `scheduler_*`). Re-run `deploy-grok-skills.py --force` after concept edits to `strict-code-review`; bundled links track xAI updates automatically. |
