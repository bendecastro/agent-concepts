...
| Pi | Deployed for `teach`, `seo` | Relative symlink: `~/.pi/agent/skills/<name>` → `../../../agents/concepts/<name>/body` from the synced CONFIG vault | Native Agent Skills metadata; unknown frontmatter ignored | Future Pi sessions discover these skills; use `/skill:<name>` to force-load if auto-selection misses. |
| Codex | `agent-kernel` delta deployed; specialized concepts via AGENTS.md reference or manual bootstrap | Kernel delta lives in `~/.codex/AGENTS.md`, Codex's documented global instruction layer. For specialized concepts, prefer an `AGENTS.md` at the right repo level that points to this workspace/concept; manual bootstrap also works. | Global/repo `AGENTS.md`; treat frontmatter as plain metadata unless the harness adds skill support | Codex merges `~/.codex/AGENTS.md` plus repo `AGENTS.md` files from root to CWD; deeper files override earlier ones. |
