...
| Grok | Deployed 2026-07-03 via `scripts/deploy-grok-skills.py` | **CONFIG → Grok:** every ingested workflow skill is a relative symlink `~/.grok/skills/<name>` → `~/Sync/CONFIG/agents/concepts/<concept>/body/` (plus `shared` → `concepts/grok-shared/shared`). Edit canon in `concepts/`; re-run deploy script. xAI's `~/.grok/bundled/skills/` is provenance only, not the live deploy source. | Native Agent Skills metadata (`when-to-use`, `disable-model-invocation`) | Grok-specific tools (`spawn_subagent`, `grok worktree rm`, `scheduler_*`). `deploy-local-skills.py` does not cover Grok — use `deploy-grok-skills.py`. |
