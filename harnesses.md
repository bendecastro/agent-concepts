...
| Grok | Bundled workflow skills auto-discovered; user skills in `~/.grok/skills/`; CONFIG concepts documented 2026-07-03 | **Bundled:** `~/.grok/bundled/skills/<name>/` (design, execute-plan, implement, pr-babysit, review) + `shared/personas/`. **User:** `~/.grok/skills/<name>/` (check-work, code-review, create-skill, …). **CONFIG canon:** reference concepts point at bundled/user paths via `body/UPSTREAM.md`; `strict-code-review` is vendored in `concepts/strict-code-review/body/`. | Native Agent Skills metadata (`when-to-use`, `disable-model-invocation`) | Grok-specific tools (`spawn_subagent`, `grok worktree rm`, `scheduler_*`) — porting to other harnesses needs tool mapping, not symlink-only deploy. Optional: symlink `strict-code-review` body into `~/.grok/skills/` if renaming away from upstream `code-review`. |
...
- Whether `strict-code-review` should replace or shadow live `~/.grok/skills/code-review/` via symlink.
- Gemini durable instruction-discovery mechanism worth supporting.
