...
| OpenCode | `agent-kernel` delta deployed; local concept skills bulk-deployed via `~/.agents/skills/` | Kernel delta lives in `~/.config/opencode/AGENTS.md`; `scripts/deploy-local-skills.py` creates relative canonical-body symlinks on OpenCode's auto-loaded shared skills bus. | Global `AGENTS.md` plus native Agent Skills metadata | Verified 2026-07-17 in a live OpenCode session: all 32 canonical `concepts/*/body/SKILL.md` skills were advertised from `~/.agents/skills/`; all links resolved. Restart sessions after deploying changes because the advertised list is loaded at startup. |
